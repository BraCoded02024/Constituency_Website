          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Icon</label><select value={form.icon} onChange={e => setForm(f => ({ ...f, icon: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-npp-blue outline-none">{ICONS.map(i => <option key={i}>{i}</option>)}</select></div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm divide-y divide-gray-50">
        {items.map(item => (
          <div key={item.id} className="p-4 flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-npp-blue/10 flex items-center justify-center text-npp-blue text-xs font-bold">{item.icon.charAt(0)}</div>
